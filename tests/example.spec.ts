...
test.describe("Movies Page", () => {
  test("items page has correct title and visible items", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="movie-card-title"]').first(),
    ).toBeVisible();
  });
  ...
  test.describe("Movie page", () => {
    test("click on movie navigates to details page", async ({ page }) => {
      ...
      const firstCardTitle = await page
        .locator('[data-testid="movie-card-title"]')
        .first()
        .innerText();
...
test.describe("Language switching", () => {
  ...
  test("page heading is in english", async ({ page }) => {
    ...
    await expect(page.locator("h2").first()).toContainText(
      "Discover the world of the best movies.",
    );
  });
  ...
  test("page heading is in german", async ({ page }) => {
    ...
    await expect(page.locator("h2").first()).toContainText(
      "Entdecken Sie die Welt der besten Filme.",
    );
  });
});
